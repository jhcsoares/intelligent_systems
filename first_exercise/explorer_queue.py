...
class Explorer(AbstAgent):
    def __init__(self, env, config_file, resc, direction=None):
        ...
        self.walk_queue = Queue()  # a stack to store the movements
    ...
    def set_walk_queue(self):
        ...
        for i in range(0, 8):
            ...
            if obstacles[index] == VS.CLEAR and coordinates not in explored_coordinates_list:
                self.walk_queue.enqueue(coordinates)
                has_enqueued = True
        
        return has_enqueued
        
    def explore(self):
        has_enqueued = self.set_walk_queue()

        if has_enqueued:
            xf, yf = self.walk_queue.dequeue()
            dx = xf - self.x
            dy = yf - self.y
